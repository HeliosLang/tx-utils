...
describe("IrisClient", async () => {
    ...
    await it("get parameters return object with expected fields", async () => {
        ...
        assert(Object.keys(params).length > 5, "expected at least 5 entries in network parameters")
    })
    ...
    await it("getUtxo() throws a UtxoAlreadySpentError for a UTXO that is known to be already spent", async () => {
        const utxoID = "33b2cad72d0f0ddafcfc16fabcf92d2ab0e9d4034ea40ab1bc1f4dfffb15fbc9#1"
        ...
        try {
            ...
            if ("consumedBy" in e) {
                assert(e.consumedBy.isEqual(makeTxId("2a710139fe3d83dc16f9dd3e9e267a98a38d8d5c23ab8b4742f0c0cc8a947ef0")))
            } else {
                throw e
            }
        }
    ...
    await it("getUtxosWithAssetClass() returns some UTXOs with known assets", async () => {
        const addr = "addr_test1wqyp8f3s30t0kvqa3vfgq8lrhv7vtxrn9w9k9vh5s4syzacyjcr9g"
        const asset = makeAssetClass(makeMintingPolicyHash("1fd38a7e152b5033e6b2f45447cbb98683a7214f3ab2b71b9ffaa04b"), hexToBytes("7450424720737570706c79"))

        const utxos = await client.getUtxosWithAssetClass(makeAddress(addr), asset)
        strictEqual(utxos.length, 1, "expected more than 0 UTXOs, got 0")
        assert(
            utxos.every((utxo) => utxo.address.isEqual(makeAddress(addr)) && utxo.value.assets.getAssetClassQuantity(asset) == 1n),
    ...
    await it("getAddressesWithAssetClass() returns a single address for a known NFT", async () => {
        ...
        assert(addresses.every(addr => addr.address.isEqual(makeAddress("addr_test1wqyp8f3s30t0kvqa3vfgq8lrhv7vtxrn9w9k9vh5s4syzacyjcr9g"))))
    })
})
